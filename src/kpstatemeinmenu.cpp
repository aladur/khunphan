...
void KPstateMainMenu::Initialize(KPstateContext *pContext,  const KPstate *pOldState)
{
...
void  KPstateMainMenu::KeyPressed (KPstateContext *pContext, unsigned char key, int x, int y)
{
  CHECK_DEFAULT_KEY_PRESSED(pContext, key, x, y);
}

void  KPstateMainMenu::MouseClick (KPstateContext *pContext, int button, int state, int x, int y)
{
  int Signal = KPstate::EvaluateMouseClick(pContext, button, state, x, y);

  switch (Signal) {
    case S_NEW_GAME:      KPConfig::Instance().SavedGame = 0;
    case S_CONTINUE:      pContext->ChangeState(KPState_Game);         break;
    case S_TUTORIAL:      pContext->ChangeState(KPState_Tutorial1);    break;
    case S_KEYBOARD_HELP: pContext->ChangeState(KPState_KeyboardHelp); break;
    case S_SETTINGS:      pContext->ChangeState(KPState_Settings);     break;
    case S_SCORELIST:     pContext->ChangeState(KPState_ScoreList);    break;
    case S_FINISH:        pContext->ChangeState(KPState_Finish);       break;
  }
}
