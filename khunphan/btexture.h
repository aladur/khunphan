...
public: 
  ...
  bool        SetTexels (const char *texels, unsigned int width, unsigned int height, unsigned int channels, int format);
  ...
  bool        CreateSubImage(int x, int y, int width, int height, char *texels);
  const char *Rescale(int exp, int format);
  void        fprintInfo(FILE *fp);

  inline unsigned int GetWidth()    { return width;     };
  inline unsigned int GetHeight()   { return height;    };
  inline unsigned int GetRowBytes() { return rowbytes;  };
  inline unsigned int GetChannels() { return channels;  };
  inline int          GetBitDepth() { return bit_depth; };
 
  static int   GetExpToBase2(unsigned int value);
  static bool  IsPowerOf2(unsigned int value);
 
  void SetChannels(unsigned int value) { channels = value; rowbytes = width * channels; };
  void SetWidth   (unsigned int value) { width    = value; rowbytes = width * channels; };
  void SetHeight  (unsigned int value) { height   = value; };

  static bool CheckFileFormat(const char *pFile); // Check if file is available and
