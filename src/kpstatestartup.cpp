...
void KPstateStartUp::Initialize(KPstateContext *pContext,  const KPstate *pOldState)
{
  KPstate::Initialize(pContext, pOldState);

  // Do some initialization stuff here:
  if (KPConfig::Instance().SkipProgressBar != 0) fullInitialized = true;
...
void KPstateStartUp::UpdateDisplay(KPstateContext *pContext)
{
  ...
  float y = 3.0f;
  menu.TextfeldArray[T_COPYRIGHT1]->Positioniere(8, y, 0.4f, A_MITTE); y -= 0.4f;
  menu.TextfeldArray[T_COPYRIGHT2]->Positioniere(8, y, 0.4f, A_MITTE); y -= 0.4f;
  menu.TextfeldArray[T_COPYRIGHT3]->Positioniere(8, y, 0.4f, A_MITTE); y -= 0.8f;
  menu.TextfeldArray[T_COPYRIGHT4]->Positioniere(8, y, 0.4f, A_MITTE); y -= 0.4f;
  menu.TextfeldArray[T_COPYRIGHT5]->Positioniere(8, y, 0.4f, A_MITTE); y -= 0.4f;
  ...
  menu.TextfeldArray[T_VERSION   ]->Positioniere(0.1f, 0,   0.5, A_LINKS);
...
void KPstateStartUp::Draw(KPstateContext *pContext)
{
  ...
  if (fullInitialized) pContext->GetBoardView().Draw();
...
void KPstateStartUp::Update(KPstateContext *pContext, int factor)
{
  ...
  if (!fullInitialized)
    pContext->GetMenu().progressBar.SetPercentage(KPnode::SetSolveCountPercentFinished());
