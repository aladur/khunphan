...
inline void KPboard::SetPosition(tKPTokenID aTokenId, int x, int y, int xExtend, int yExtend)
{
...
bool KPboard::Move(tKPTokenID aTokenID, tKPDirection d)
{
  ...
  id = 0;
	...
	case MOVE_DOWN:
		if (y+yExtend >= VERTICAL_MAX) return false;
		for (i = 0; i < xExtend; i++)
	...
	case MOVE_RIGHT:
		if (x+xExtend >= HORIZONTAL_MAX) return false;
		for (j = 0; j < yExtend; j++)
