...
const char *BTexture::ReadTextureFromPngFile(FILE *fp, int flags)
{
  ...
  //if ((format & TEX_WITH_ALPHA) && bit_depth == 8 && color_type == PNG_COLOR_TYPE_RGB)
...
const char *BTexture::CopyAlphaChannel(int format)
{
  ...
  if (format & TEX_MODIFY_ALPHA)
    for (i = 0; i < rowbytes * height; i += channels)
    {
      ...
      if (channels > 1 && (format & TEX_MODIFY_ALPHA) == TEX_USE_GREEN_FOR_ALPHA)
  ...
  if (format & TEX_COPY_COLOR)
    for (i = 0; i < rowbytes * height; i += channels)
    {
      if (channels > 1 && (format & TEX_COPY_COLOR) == TEX_COPY_RED_TO_GREEN)
        texels[i+1] = texels[i];
      if (channels > 2 && (format & TEX_COPY_COLOR) == TEX_COPY_RED_TO_BLUE)
        texels[i+2] = texels[i];
      if (channels > 1 && (format & TEX_COPY_COLOR) == TEX_COPY_GREEN_TO_RED)
        texels[i]   = texels[i+1];
      if (channels > 2 && (format & TEX_COPY_COLOR) == TEX_COPY_GREEN_TO_BLUE)
        texels[i+2] = texels[i+1];
      if (channels > 2 && (format & TEX_COPY_COLOR) == TEX_COPY_BLUE_TO_RED)
        texels[i]   = texels[i+2];
  ...
  if (format & TEX_SET_COLORS)
    for (i = 0; i < rowbytes * height; i += channels)
    {
      if ((format & TEX_SET_COLOR_RED) == TEX_SET_COLOR_RED)
        texels[i] = red;
      if (channels > 1 && (format & TEX_SET_COLOR_GREEN) == TEX_SET_COLOR_GREEN)
        texels[i+1] = green;
...
bool BTexture::CreateSubImage(int x, int y, int w, int h, char *subTexels)
{
  ...
  {
    memcpy(&subTexels[i], &texels[ay * rowbytes + (x * channels)], w * channels);
    i += w * channels;
  }
  return true;
}
...
void BTexture::fprintInfo(FILE *fp)
{
  ...
  switch (color_type)
  {
    case PNG_COLOR_TYPE_GRAY:       pColorType = (char *)"PNG_COLOR_TYPE_GRAY";       break;
    case PNG_COLOR_TYPE_PALETTE:    pColorType = (char *)"PNG_COLOR_TYPE_PALETTE";    break;
    case PNG_COLOR_TYPE_RGB:        pColorType = (char *)"PNG_COLOR_TYPE_RGB";        break;
    case PNG_COLOR_TYPE_RGB_ALPHA:  pColorType = (char *)"PNG_COLOR_TYPE_RGB_ALPHA";  break;
    case PNG_COLOR_TYPE_GRAY_ALPHA: pColorType = (char *)"PNG_COLOR_TYPE_GRAY_ALPHA"; break;
    default:                        pColorType = (char *)"???";
  }
  switch (interlace_type)
  {
    case PNG_INTERLACE_NONE:        pInterlaceType = (char *)"PNG_INTERLACE_NONE";  break;
    case PNG_INTERLACE_ADAM7:       pInterlaceType = (char *)"PNG_INTERLACE_ADAM7"; break;
    case PNG_INTERLACE_LAST:        pInterlaceType = (char *)"PNG_INTERLACE_LAST";  break;
    default:                        pInterlaceType = (char *)"???";
  }
  switch (compression_type)
  {
    case PNG_COMPRESSION_TYPE_BASE:   pCompressionType = (char *)"PNG_COMPRESSION_TYPE_BASE";    break;
    default:                          pCompressionType = (char *)"???";
  }
...
bool BTexture::SetTexels(const char *pTexels, unsigned int aWidth, unsigned int aHeight, unsigned int aChannels, int format)
{
  delete [] texels;
  texels = NULL;
  ...
  switch (format)
  {
    case  TEX_ILLUMINANCE:       color_type = PNG_COLOR_TYPE_GRAY;       break;
    case  TEX_ILLUMINANCE_ALPHA: color_type = PNG_COLOR_TYPE_GRAY_ALPHA; break;
    case  TEX_RGB:               color_type = PNG_COLOR_TYPE_RGB;        break;
    case  TEX_RGB_ALPHA:         color_type = PNG_COLOR_TYPE_RGB_ALPHA;  break;
  ...
  if ((format & TEX_SMALLER) != 0)
  {
    ...
    if ((format & TEX_RESCALE_AVG) != 0)
    {
         ...
         for(x = 0; x < GetWidth(); x++)
         {
           ...
           for (oy = 0; oy < factor; oy++)
           {
              ...
              for (ox = 0; ox < fChannels; ox++)
              {
                ...
                if (++c == GetChannels()) c = 0;
              }
           }
           switch (GetChannels())
           {
             case 1: *(pRow++) = pBuffer[0] >> shift; break;
             case 2: *(pRow++) = pBuffer[0] >> shift;
                     *(pRow++) = pBuffer[1] >> shift; break;
             case 3: *(pRow++) = pBuffer[0] >> shift;
                     *(pRow++) = pBuffer[1] >> shift;
                     *(pRow++) = pBuffer[2] >> shift; break;
             case 4: *(pRow++) = pBuffer[0] >> shift;
                     *(pRow++) = pBuffer[1] >> shift;
                     *(pRow++) = pBuffer[2] >> shift;
                     *(pRow++) = pBuffer[3] >> shift; break;
             default:
      ...
      for (y = 0; y < GetHeight(); y++)
      {
         ...
         for(x = 0; x < GetWidth(); x++)
         {
           memcpy(&pRow[x * GetChannels()], &pOldRow[(x * GetChannels()) << exp], GetChannels());
         }
      }
    }
    delete [] oldTexels;
