...
void KPstateGame::Initialize(KPstateContext *pContext,  const KPstate *pOldState)
{
  ...
  if (KPConfig::Instance().SavedGame != 0) {
    ...
    pContext->GetStatistics().SetEventCounter(MOVE_COUNTER,       KPConfig::Instance().Moves        );
    pContext->GetStatistics().SetEventCounter(MOVE_WITH_HELP_CNT, KPConfig::Instance().MovesWithHint);
    pContext->GetStatistics().SetEventCounter(USED_CHEATS_CNT,    KPConfig::Instance().CheatCount   );
  } else
...
void KPstateGame::UpdateDisplay(KPstateContext *pContext)
{
  ...
  pContext->GetUserInterface().SetSoundVolume(KPConfig::Instance().SoundOn ? KPConfig::Instance().SoundVolume : 0);
  ...
  pContext->GetMenu().TextfeldArray[T_MINMOVECOUNT]->FormatText(NULL, pContext->GetBoardView().GetMovesToSolve());
  pContext->GetMenu().TextfeldArray[T_MOVECOUNT   ]->FormatText(NULL, pContext->GetStatistics().GetEventCounter(MOVE_COUNTER));

  if (KPConfig::Instance().SolutionHint)
    menu.TextfeldArray[T_MINMOVECOUNT]->Positioniere(7,0.1f,0.6f,A_LINKS);

  menu.TextfeldArray[T_MOVECOUNT]->Positioniere(4,0.1f,0.6f,A_LINKS);
  menu.TextfeldArray[T_TIME     ]->Positioniere(12.5,0.1f,0.6f,A_LINKS);
...
void  KPstateGame::KeyPressed (KPstateContext *pContext, unsigned char key, int x, int y)
{
  if (bIsPause)
  {
  ...
  // means Ctrl+X
    case 'X' - 'A' + 1: Cheat1(pContext); break;
...
void KPstateGame::Update(KPstateContext *pContext, int factor)
{
  ...
  if (counter > TOTAL_ANIMATIONTIME) {
    ...
    pContext->GetMenu().TextfeldArray[T_TIME]->FormatText(NULL,
        (const char *)pContext->GetStatistics().GetTotalTime(RTIME_HHMMSS));     
  }
}
...
void KPstateGame::SaveGameStatus(KPstateContext *pContext)
{
  ...
  KPConfig::Instance().Moves         = pContext->GetStatistics().GetEventCounter(MOVE_COUNTER      );
  KPConfig::Instance().MovesWithHint = pContext->GetStatistics().GetEventCounter(MOVE_WITH_HELP_CNT);
  KPConfig::Instance().CheatCount    = pContext->GetStatistics().GetEventCounter(USED_CHEATS_CNT   );
}
