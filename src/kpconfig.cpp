...
std::string KPConfig::GetDirectory(tKPDir directoryID) const
{
  ...
  switch (directoryID)
  {
    case KP_TEXTURE_DIR: return "Textures\\";
    case KP_SOUND_DIR:   return "Sounds\\";
    case KP_MUSIC_DIR:   return "Music\\";
    case KP_LOCALE_DIR:  return "locale\\";
    default: return "";
  }
#endif
#ifdef LINUX
  ...
  switch (directoryID)
  {
    case KP_TEXTURE_DIR: dir = "Textures/"; break;
    case KP_SOUND_DIR:   dir = "Sounds/";   break;
...
void KPConfig::WriteToFile()
{
    ...
    doc->children = xmlNewDocNode( doc, NULL, (const xmlChar *)"KhunPhan", NULL );
    xmlSetProp( doc->children, (const xmlChar *)"Version", (const xmlChar *)VERSION );
...
void KPConfig::ReadFromFile()
{
    ...
    if (cur)
    {
      ...
      while (tree != NULL) {
        ...
        if ((!xmlStrcmp(tree->name, (const xmlChar *)"GameControl")) && (cur->ns == ns)) {
           subtree = tree->xmlChildrenNode;
           while (subtree != NULL) {
        ...
        if ((!xmlStrcmp(tree->name, (const xmlChar *)"AudioSettings")) && (cur->ns == ns)) {
           subtree = tree->xmlChildrenNode;
           while (subtree != NULL) {
        ...
        if ((!xmlStrcmp(tree->name, (const xmlChar *)"LanguageSettings")) && (cur->ns == ns)) {
           subtree = tree->xmlChildrenNode;
           while (subtree != NULL) {
        ...
        if ((!xmlStrcmp(tree->name, (const xmlChar *)"DebugSettings")) && (cur->ns == ns)) {
           subtree = tree->xmlChildrenNode;
           while (subtree != NULL) {
        ...
        if ((!xmlStrcmp(tree->name, (const xmlChar *)"SavedGames")) && (cur->ns == ns)) {
           subtree = tree->xmlChildrenNode;
           while (subtree != NULL) {
              if ((!xmlStrcmp(subtree->name, (const xmlChar *)"SavedGame")) && (cur->ns == ns)) {
