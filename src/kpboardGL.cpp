...
bool KPboardView::CreateTexture(unsigned int TextureSize, const char *pFile, bool Nearest, unsigned int *pId)
{
...
bool KPboardView::Initialize(const char *TextureName, unsigned int TextureSize /*=1*/, bool Nearest /*=true*/)
{
  ...
  CreateCuboid(
    ...
    -((2 * (DX + DGAP) - DGAP) / 2) - DGAP,
     ((VERTICAL_MAX   * (DY + DGAP) - DGAP) / 2) + DGAP, (DZ * 2 / 3) + DGAP);
  CreateCuboid(
    DBOXW + DX + DGAP, DBOXW, DBOXH,
     ((2 * (DX + DGAP) - DGAP) / 2) + DGAP,
     ((VERTICAL_MAX   * (DY + DGAP) - DGAP) / 2) + DGAP, 0);
  ...
  // Bottom end
  CreateCuboid(
    2 * DBOXW + DGAP + (HORIZONTAL_MAX * (DX + DGAP)), 2 * DBOXW + DGAP + VERTICAL_MAX * (DY + DGAP), DBOXW,
...
void KPboardView::CreateCuboid(float dx, float dy, float dz, float x0, float y0, float z0, bool WithTexture /* = true */) const
{
  ...
  GLfloat nv[] = {0,0,0};
          ...
          tdx = tx0 + dx / DT;  tdy = ty0 + dy / DT;
          nv[0] = 0; nv[1] = 0; nv[2] = 1;
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, ty0); glVertex3f( x0,      y0,      z0 + dz);
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, ty0); glVertex3f( x0 + dx, y0,      z0 + dz);
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, tdy); glVertex3f( x0 + dx, y0 + dy, z0 + dz);
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, tdy); glVertex3f( x0,      y0 + dy, z0 + dz);

					// Back Face
          tx0 = getRnd(); ty0 = getRnd();
          tdx = tx0 + dx / DT; tdy = ty0 + dz / DT;
          nv[0] = 0; nv[1] = -1; nv[2] = 0;
          glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, ty0); glVertex3f( x0,      y0,      z0     );
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, ty0); glVertex3f( x0 + dx, y0,      z0     );
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, tdy); glVertex3f( x0 + dx, y0,      z0 + dz);
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, tdy); glVertex3f( x0,      y0,      z0 + dz);

					// Front Face
          tx0 = getRnd(); ty0 = getRnd();
          tdx = tx0 + dz / DT; tdy = ty0 + dx / DT;
          nv[0] = 0; nv[1] = 1; nv[2] = 0;
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, ty0); glVertex3f( x0,      y0 + dy, z0     );
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, ty0); glVertex3f( x0,      y0 + dy, z0 + dz);
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, tdy); glVertex3f( x0 + dx, y0 + dy, z0 + dz);
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, tdy); glVertex3f( x0 + dx, y0 + dy, z0     );

					// Left Face
          tx0 = getRnd(); ty0 = getRnd();
          tdx = tx0 + dz / DT; tdy = ty0 + dy / DT;
          nv[0] = -1; nv[1] = 0; nv[2] = 0;
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, ty0); glVertex3f( x0,      y0,      z0     );
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, ty0); glVertex3f( x0,      y0,      z0 + dz);
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, tdy); glVertex3f( x0,      y0 + dy, z0 + dz);
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, tdy); glVertex3f( x0,      y0 + dy, z0     );

					// Right Face
          tx0 = getRnd(); ty0 = getRnd();
          tdx = tx0 + dy / DT; tdy = ty0 + dz / DT;
          nv[0] = 1; nv[1] = 0; nv[2] = 0;
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, ty0); glVertex3f( x0 + dx, y0,      z0     );
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, ty0); glVertex3f( x0 + dx, y0 + dy, z0     );
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, tdy); glVertex3f( x0 + dx, y0 + dy, z0 + dz);
  ...
  /* The bottom face is never displayed. It increases performance not to draw it.
          ...
          nv[0] = 0; nv[1] = 0; nv[2] = -1;
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, ty0); glVertex3f( x0,      y0,      z0     );
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, ty0); glVertex3f( x0,      y0 + dy, z0     );
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tdx, tdy); glVertex3f( x0 + dx, y0 + dy, z0     );
					glNormal3fv(nv); if (WithTexture) glTexCoord2f(tx0, tdy); glVertex3f( x0 + dx, y0,      z0     );
*/
					glEnd();
...
void KPboardView::GetTokenCoordinates(tKPTokenID i, float *x, float *y) const
{
  if ( i == animatedToken ) {
    ...
    if (x != NULL) *x = ((DX + DGAP) * (GLfloat)current.GetX(i) - ((HORIZONTAL_MAX * (DX + DGAP) - DGAP) / 2));
    if (y != NULL) *y = ((DY + DGAP) * (GLfloat)current.GetY(i) - ((VERTICAL_MAX   * (DY + DGAP) - DGAP) / 2));
  }
}
...
void KPboardView::DrawToken(const tKPTokenID i) const
{
  switch (i)
  {
    case TK_GREEN1: glCallList(callList + CUBOID_1X1_1); break;
    case TK_GREEN2: glCallList(callList + CUBOID_1X1_2); break;
    case TK_GREEN3: glCallList(callList + CUBOID_1X1_3); break;
    case TK_GREEN4: glCallList(callList + CUBOID_1X1_4); break;
    case TK_WHITE1: glCallList(callList + CUBOID_1X2_1); break;
    case TK_WHITE2: glCallList(callList + CUBOID_1X2_2); break;
    case TK_WHITE3: glCallList(callList + CUBOID_1X2_3); break;
    case TK_WHITE4: glCallList(callList + CUBOID_1X2_4); break;
    case TK_WHITE5: glCallList(callList + CUBOID_2X1);   break;
    case TK_RED1:   glCallList(callList + CUBOID_2X2);   break;
    default:
      break;
  }
}
...
{
  if ( animatedToken != TK_EMPTY ) return false; // Ignore if Animation in progress
  short int i = 0;
  tKPDirection direction = MOVE_NO;
  // count the possibilities token id can move to in i
  if ( current.CanMove(id, MOVE_UP  )  ) { i++; direction = MOVE_UP;    }
  if ( current.CanMove(id, MOVE_DOWN)  ) { i++; direction = MOVE_DOWN;  }
  if ( current.CanMove(id, MOVE_LEFT)  ) { i++; direction = MOVE_LEFT;  }
  if ( current.CanMove(id, MOVE_RIGHT) ) { i++; direction = MOVE_RIGHT; }
  if ( i == 0 ) return false;  // token is not movable at all
  if ( i >= 2 ) direction = d; // more than one possibility: use mouse motion to get direction
  if ( direction == MOVE_NO ) return false; // no direction given. No move possible
  if ( !current.CanMove(id, direction) ) return false; // Ignore if token id can not be moved in direction d

  animatedToken = id;
  Time = 0;
  ax = old_x = ((DX + DGAP) * (GLfloat)current.GetX(id) - ((HORIZONTAL_MAX * (DX + DGAP) - DGAP) / 2));
  ay = old_y = ((DY + DGAP) * (GLfloat)current.GetY(id) - ((VERTICAL_MAX   * (DY + DGAP) - DGAP) / 2));
  
  current.Move(id, direction); // do the move

  new_x = ((DX + DGAP) * (GLfloat)current.GetX(id) - ((HORIZONTAL_MAX * (DX + DGAP) - DGAP) / 2));
  new_y = ((DY + DGAP) * (GLfloat)current.GetY(id) - ((VERTICAL_MAX   * (DY + DGAP) - DGAP) / 2));
...
void KPboardView::DrawCuboid(float deltaAngle) const// Only for test purposes
{
  ...
  GLint without_camera = 0;  // switch to 1 to test without any camera properties
