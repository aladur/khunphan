...
void KPstateAudioSettings::Initialize(KPstateContext *pContext,  const KPstate *pOldState)
{
...
void KPstateAudioSettings::UpdateDisplay(KPstateContext *pContext)
{
  ...
  menu.TextfeldArray[textfeldIdx]->Positioniere(8.2f,y,0.71f,A_LINKS);
...
void KPstateAudioSettings::KeyPressed (KPstateContext *pContext, unsigned char key, int x, int y)
{
  CHECK_DEFAULT_KEY_PRESSED(pContext, key, x, y);
}

void  KPstateAudioSettings::MouseClick (KPstateContext *pContext, int button, int state, int x, int y)
{
  int Signal = KPstate::EvaluateMouseClick(pContext, button, state, x, y);

  switch (Signal) {
    case S_TOGGLE_SOUND_VOLUME: ToggleSoundVolume(pContext); UpdateDisplay(pContext); break;
    case S_TOGGLE_MUSIC_VOLUME: ToggleMusicVolume(pContext); UpdateDisplay(pContext); break;
