...
Textfeld::Textfeld() : pFormat(NULL), pString(NULL), size(0),
                       x(0), y(0), Hoehe(0), Aspekt(0), Alpha(0),
...
void Textfeld::CheckValidString(int min_size, const char *srcString /*= NULL */)
{
  ...
  {
    int m = min_size;
    if (m < 50) m = 50; // define a min size of 50 (avoiding mem. allocation)
    sTmp = pString;
    pString = new char [m];
    size = m;
    pString[0] = '\0';
...
const char* Textfeld::Text(){return pString;}

bool Textfeld::IsDeactivated() const
{
  return ((soll_Alpha != Alpha && (soll_Alpha == MOD_AUSGEBLENDET)) || (Alpha == MOD_AUSGEBLENDET));
}
...
void Textfeld::GeneriereDisplayList()
{
  ...
  if (!maxWidth)
  {
    ...
    while ((c = (unsigned char)pString[p++]) != '\0')
    {
      ...
      x = c % 16;
      y = 15 - (c / 16);
      glTexCoord2f(w*x,     w*y);     glVertex2f(0.0,0.0);
      glTexCoord2f(w*(x+1), w*y);     glVertex2f(1.0,0.0);
  ...
  if (maxWidth) {
    ...
    while (pString[Pos]) {
      ...
      while ((character = (unsigned char)pString[Pos]) && lineWidth < maxWidth * 1.05)
      {
        if (character==32) {
