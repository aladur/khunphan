...
void KPprogressBar::SetPosition(float X, float Y, float Width, float Height, int Alignment)
{
  switch (Alignment)
  {
    case A_LINKS: { target_x = X;               } break;
    case A_MITTE: { target_x = X - Width * 0.5f; } break;
    case A_RECHTS:{ target_x = X - Width;       } break;
  }
...
bool KPprogressBar::Animate(int factor)
{
  ...
  if (Time >= TOTAL_ANIMATIONTIME) {
    ...
    GLfloat factor = 0.5f - 0.5f * (GLfloat)cos(M_PI * Time / TOTAL_ANIMATIONTIME);
